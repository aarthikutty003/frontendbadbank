        ...
        try{
            if (isNaN(withdraw) || withdraw < 0) {
              ...
              alert('insufficient account balance')
                ...
                await axios.put(`https://serve-ad8f.onrender.com/update/${id}`, { amount: total - withdraw }).then(setTotal(total - withdraw)).then(alert(`amount ${withdraw} is withdraw successfully !`));
               setWithdraw('');
            }
        }
        catch(err){
            ...
            <>
                ...
                <h3>Account Balance: {total} </h3>
                    <Form className="form-inline" onSubmit={submitHandler}>
                         ...
                         <Form.Group className="mb-3">
                            <Form.Control type="number" placeholder="Enter amount" onChange={(e) => changeHandler(Number(e.target.value), setWithdraw)} value={withdraw} />
                        </Form.Group>
                        <Button type='submit' className='create-btn disabled' style={{ backgroundColor: 'black', color: 'white', borderColor: 'white' }}>Withdraw</Button>
                    </Form>
                </Card> </>
        </div>
        </div>
...
//                 await axios.put(`http://localhost:8081/update/${id}`, { amount: total - withdraw }).then(setTotal(total - withdraw)).then(alert(`amount ${withdraw} is withdrawn successfully !`));
...
//                         </Form.Group>
//                         <Button type='submit' id='btn'className="mb-3">Deposit</Button>
//                     </Form>
//                 </Card> </>
//         </div>
//         </div>
